...
namespace OPS.Serialization.Byte
{
    internal class Buffer
    {
        ...
        public Buffer()
        {
            this.m_Buffer = new byte[k_InitialSize];
        }
        ...
        private void WriteCheckForSpace(int count)
        {
            if (this.m_Pos + count >= this.m_Buffer.Length)
            {
                int num = (int)Math.Ceiling((double)((float)this.m_Buffer.Length * k_GrowthFactor));
                while (this.m_Pos + count >= num)
                {
                    num = (int)Math.Ceiling((double)((float)num * k_GrowthFactor));
                    if (num > k_BufferSizeWarning)
                    {
                        //Debug.LogWarning("Buffer size is " + num + " bytes!");
                        Console.WriteLine("Buffer size is " + num + " bytes!");
                    }
                }
